...
const getStats = async () => {
    ...
    return {
        revenue: { total: current.revenue, lastMonth: last.revenue },
        success: { total: current.count, lastMonth: last.count },
        pending: { total: pending.total, newToday: pending.newToday },
    };
};
...
const getList = async (search, status, page = 1, limit = 20) => {
    ...
    if (status) matchCondition.paymentStatus = status;
    ...
    if (search) {
        aggregation.push(
            {
                $addFields: {
                    studentFullName: { $concat: ["$student.name", " ", { $ifNull: ["$student.lastName", ""] }] },
                },
            },
            {
                $match: {
